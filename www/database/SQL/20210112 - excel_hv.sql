drop table if exists  esclarecimiento.excel_hv;
create table esclarecimiento.excel_hv
(
    id serial not null
        constraint excel_hv_pkey
        primary key,
    ...
    entrevistado_otros_nombres text,
    entrevistado_sexo text,
    entrevistado_orientacion_sexual text,
    entrevistado_identidad_genero text,
    entrevistado_pertenencia_etnico_racial text,
    titulo text,
    observaciones text,
    dinamica_1 text,
    dinamica_2 text,
...
comment on column esclarecimiento.excel_hv.id_historia_vida is 'Trazabilidad a tabla original';


alter table esclarecimiento.excel_hv owner to dba;

grant select on esclarecimiento.excel_hv to solo_lectura;

create index excel_hv_id_entrevistador_index
    on esclarecimiento.excel_hv (id_entrevistador);

create unique index excel_hv_codigo_entrevista_uindex
    on esclarecimiento.excel_hv (codigo_entrevista);
create index excel_hv_id_historia_vida_index
    on esclarecimiento.excel_hv (id_historia_vida);
