...
create table if not exists esclarecimiento.excel_personas_entrevistadas
(
    id_excel_personas_entrevistadas serial
...
comment on column esclarecimiento.excel_personas_entrevistadas.id_consentimiento is 'id_entrevista para nombres obtenidos del consentimiento informado';

alter table esclarecimiento.excel_personas_entrevistadas owner to dba;

create index if not exists eps_fts_gin_index
    on esclarecimiento.excel_personas_entrevistadas (fts);

create index if not exists excel_personas_entrevistadas_apellidos_index
    on esclarecimiento.excel_personas_entrevistadas (apellidos);

create index if not exists excel_personas_entrevistadas_clasificacion_nivel_index
    on esclarecimiento.excel_personas_entrevistadas (clasificacion_nivel);

create index if not exists excel_personas_entrevistadas_codigo_entrevista_index
    on esclarecimiento.excel_personas_entrevistadas (codigo_entrevista);

create index if not exists excel_personas_entrevistadas_id_entrevista_lugar_index
    on esclarecimiento.excel_personas_entrevistadas (id_entrevista_lugar);

create index if not exists excel_personas_entrevistadas_id_macroterritorio_index
    on esclarecimiento.excel_personas_entrevistadas (id_macroterritorio);

create index if not exists excel_personas_entrevistadas_id_sector_index
    on esclarecimiento.excel_personas_entrevistadas (id_sector);

create index if not exists excel_personas_entrevistadas_id_sexo_index
    on esclarecimiento.excel_personas_entrevistadas (id_sexo);

create index if not exists excel_personas_entrevistadas_id_subserie_id_primaria_index
    on esclarecimiento.excel_personas_entrevistadas (id_subserie, id_entrevista);

create index if not exists excel_personas_entrevistadas_id_territorio_index
    on esclarecimiento.excel_personas_entrevistadas (id_territorio);

create index if not exists excel_personas_entrevistadas_nombres_index
    on esclarecimiento.excel_personas_entrevistadas (nombres);

create index if not exists excel_personas_entrevistadas_otros_nombres_index
