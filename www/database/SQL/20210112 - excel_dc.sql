drop table if exists  esclarecimiento.excel_dc;
create table esclarecimiento.excel_dc
(
    id serial not null
        constraint excel_dc_pkey
        primary key,
...
comment on column esclarecimiento.excel_dc.id_diagnostico_comunitario is 'Trazabilidad a tabla original';


alter table esclarecimiento.excel_dc owner to dba;

grant select on esclarecimiento.excel_dc to solo_lectura;

create index excel_dc_id_entrevistador_index
    on esclarecimiento.excel_dc (id_entrevistador);

create unique index excel_dc_codigo_entrevista_uindex
    on esclarecimiento.excel_dc (codigo_entrevista);
create index excel_dc_id_diagnostico_comunitario_index
    on esclarecimiento.excel_dc (id_diagnostico_comunitario);
