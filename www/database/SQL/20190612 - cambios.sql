...
alter table esclarecimiento.entrevistador
	add solo_lectura int default 2 not null;

comment on column esclarecimiento.entrevistador.solo_lectura is '1:Sí, else: no';

UPDATE "catalogos"."cat_cat" SET "nombre" = 'Documentos: Objetivos' WHERE "id_cat" = 6;
UPDATE "catalogos"."cat_cat" SET "nombre" = 'Documentos: Instrumentos' WHERE "id_cat" = 7;
update catalogos.cat_cat set editable=2 where id_cat in (1,2,3);
update catalogos.cat_cat set editable=1 where id_cat not in (1,2,3);