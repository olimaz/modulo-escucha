...
@section('content_header')
    <h1>
        Finalizar asignación de transcripción
    </h1>
@endsection
...
@section('content')
    ...
    <div class="box box-primary">
        ...
        {!! Form::model($transcribirAsignacion, ['route' => ['transcribirAsignacions.update', $transcribirAsignacion->id_transcribir_asignacion], 'method' => 'patch','id'=>'frm_abc']) !!}
        <div class="col-sm-6">
            <div class="box box-default box-solid">
                <div class="box-header">
                    <h3 class="box-title">1. Procesamiento de la entrevista {!! $transcribirAsignacion->codigo_entrevista !!}</h3>
                </div>
                <div class="box-body">
                    ...
                    <div class="form-group col-sm-6">
                        {!! Form::label('id_situacion', 'Resultado de la transcripción:') !!}
                        {{-- TODO: validar que hayan adjuntado --}}
                        {!! Form::select('id_situacion', \App\Models\criterio_fijo::listado_resultados_transcripcion(),null, ['class' => 'form-control','rows'=>3]) !!}
                    </div>
                    <div class="form-group col-sm-6 ">
                        @include('controles.catalogo', ['control_control' => 'id_causa'
                                         ,'control_id_cat'=>84
                                         , 'control_default'=>$transcribirAsignacion->id_causa
                                         , 'control_multiple' => false
                                         , 'control_requerido' => false
                                         , 'control_vacio' => 'No Aplica: sí se transcribió'
                                         ,'control_texto'=>'Si no fué transcrito, clasifique la causa:'])
                    </div>
                    <div class="clearfix"></div>
                    <div class="form-group col-sm-6">
                    ...
                    <!-- Observaciones Field -->
                    <div class="form-group col-sm-12 ">
                        {!! Form::label('observaciones', 'Observaciones respecto a la transcripción:') !!}
        ...
        <div class="col-sm-6">
            ...
            <div class="box box-default box-solid">
                ...
                <div class="box-body">
                    @php($default_fluidez = $transcribirAsignacion->fluidez)
                    @php($default_cierre = $transcribirAsignacion->cierre)
                    @php($default_d_hecho = $transcribirAsignacion->d_hecho)
                    @php( $default_d_contexto = $transcribirAsignacion->d_contexto)
                    @php($default_d_impacto = $transcribirAsignacion->d_impacto)
                    @php($default_d_justicia = $transcribirAsignacion->d_justicia)
                    @php($default_ahora_entiendo = $transcribirAsignacion->ahora_entiendo)
                    @php($default_cambio_perspectiva = $transcribirAsignacion->cambio_perspectiva)
...
@elseif($transcribirAsignacion->id_entrevista_colectiva)
    @include("entrevista_colectivas.js_carga_archivo_adjuntar")
@elseif($transcribirAsignacion->id_entrevista_etnica)
    @include("entrevista_etnicas.js_carga_archivo_adjuntar")
@elseif($transcribirAsignacion->id_entrevista_profundidad)
    @include("entrevista_profundidads.js_carga_archivo_adjuntar")
@elseif($transcribirAsignacion->id_diagnostico_comunitario)
    @include("diagnostico_comunitarios.js_carga_archivo_adjuntar")
@elseif($transcribirAsignacion->id_historia_vida)
    ...
    <script>
        $(function(){
            $('#btn_asignar').click(function(){
                var minutos = $("#duracion_entrevista_minutos").val();
                var minutos_trans =  $("#duracion_transcripcion_minutos").val();

                var correcto = true;
                if(minutos <= 0) {
                    Swal.fire({
                        type: 'error',
                        title: 'Duración de la entrevista',
                        text: 'Favor de especificar la duración de la entrevista como un número entero, sin signos de puntuación'
                    });

                    //alert('Favor de especificar la duración de la entrevista como un número entero, sin signos de puntuación');
                    $("#duracion_entrevista_minutos").focus();
                    correcto = false;
                }
                /*
                if(minutos_trans <= 0) {
                    alert('Favor de especificar la duración de la transcripción como un número entero, sin signos de puntuación');
                    $("#duracion_transcripcion_minutos").focus();
                    correcto = false;
                }
                */
