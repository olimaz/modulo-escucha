...
@section('content_header')
    ...
    <div class="box box-default collapsed-box box-solid">            
        ...
        <div class="box-body" style="display: none">
            ...
            <div class="form-group col-sm-4 {{$filtros->nombre ? 'has-success' : ''}}">
                {!! Form::label('nombre', 'Nombres') !!}
                {!! Form::text('nombre', null, ['class' => 'form-control', 'maxlength' => 200]) !!}
            </div>
            
            <div class="form-group col-sm-4 {{$filtros->apellido ? 'has-success' : ''}}">
                {!! Form::label('apellido', 'Apellidos') !!}
                {!! Form::text('apellido', null, ['class' => 'form-control', 'maxlength' => 200]) !!}
            </div>            

            <div class="form-group col-sm-4 {{$filtros->alias ? 'has-success' : ''}}">
            ...
            <!-- Num Documento Field -->
            <div class="form-group col-sm-4 {{$filtros->num_documento ? 'has-success' : ''}}">
            ...
            <div class="form-group col-sm-4 {{$filtros->correo_electronico ? 'has-success' : ''}}">
            ...
            <div class="form-group col-sm-4 {{$filtros->profesion ? 'has-success' : ''}}">
                {!! Form::label('profesion', 'Profesión') !!}
                {!! Form::text('profesion', $filtros->profesion, ['class' => 'form-control', 'maxlength' => 100]) !!}
            </div>

            <div class="form-group col-sm-4 {{$filtros->ocupacion_actual ? 'has-success' : ''}}">
            ...
            <div class="col-sm-4">
                @include('controles.catalogo', ['control_control' => 'discapacidad'
                                                    ,'control_id_cat'=>44
                                                    , 'control_default'=>$filtros->discapacidad
            ...
            <div class="form-group col-md-6">
                @include('controles.geo3', ['control_control' => 'id_lugar_nacimiento'
            ...
            <div class="form-group col-md-6" style="margin-top:1.6%">
                @include('controles.fecha_incompleta', ['control_control' => 'fec_nac'
                                                     , 'control_default'=>$filtros->fec_nac
                                                     , 'control_vacio' => '[Mostrar todos]'
                                                     , 'control_resaltar' => true
            ...
            <div class="form-group col-sm-4 {{$filtros->cargo_publico > 0 ? 'has-success' : ''}}">                    
                {!! Form::label('Ejerce autoridad o cargo público', 'Ejerce autoridad o cargo público:') !!}
                {!! Form::select('cargo_publico', ['1'=>'Si', '2'=>'No', '-1'=>'[Mostrar todos]'], null, ['class' => 'form-control']) !!}
            </div>
            ...
            <div class="clearfix"></div>
            <div class="text-left">
            <a href="{{route('buscar_victimas')}}" class="btn btn-default">Quitar filtros y mostrar todo</a>
...
@section('content')
    ...
    <h3>
        Víctimas 
        <small> {{ $listado->total() }} en total</small>
    </h3>
            ...
            <table class="table table-condensed table-hover">
                ...
                <tbody>
                    @if (count($listado)>0)
                        ...
                        @php($link = "style='cursor: pointer;' onclick='window.location.href=".'"'.route('victimas.show', [$fila->id_persona, $fila->id_e_ind_fvt])."?search".'"'.";'")                  
                            <tr>
                                <td {!! $link !!}>{!! $fila->fmt_id_e_ind_fvt !!}</td>
                                <td {!! $link !!}>{!! $fila->nombre !!}</td>
                                <td {!! $link !!}>{!! $fila->apellido !!}</td>
                                <td {!! $link !!}>{!! $fila->alias !!}</td>
                                <td {!! $link !!}>{!! $fila->documento_identidad !!}</td>
                                <td {!! $link !!}>{!! $fila->sexo !!}</td>
                                <td {!! $link !!}>{!! $fila->fechaNacimiento !!}</td>
                                <td>
                                    <div class='btn-group'>
                                        <a href="{!! route('victimas.show', [$fila->id_persona, $fila->id_e_ind_fvt])."?search" !!}" class='btn btn-default btn-sm'><i class="glyphicon glyphicon-eye-open"></i></a>
                                        <a data-toggle="tooltip" title="Ir a fichas"  href="{!! action('entrevista_individualController@fichas', [$fila->id_e_ind_fvt]) !!}" class='btn btn-default btn-sm'><i class="glyphicon glyphicon-send"></i></a>
                                    </div>
                                </td>                                
                            </tr>                        
                        @endforeach                    
