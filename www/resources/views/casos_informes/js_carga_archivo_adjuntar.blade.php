...
@push('js')
    ...
    <script>
        ...
        function upload(img) {
            ...
            form_data.append('id_expediente',{{ $casosInformes->id_casos_informes }});


            $("#btn_grabar").prop("disabled",true);
            $("#btn_grabar").val(ico_cargando_boton);
            $.ajax({
                url: "{{url('/upload_adjuntar_caso_informe')}}",
                data: form_data,
                type: 'POST',
                dataType: "json",
                contentType: false,
        ...
        function actualizar_tabla() {
            $.ajax({
                url: " {{ url("/tabla_adjuntos_casos/".$casosInformes->id_casos_informes) }} ",
                type: 'GET',
                success: function (data) {
