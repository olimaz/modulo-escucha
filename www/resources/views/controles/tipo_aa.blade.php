...
Selector de dos niveles para Actore Armados
    Parametros:
...
<?php
    ...
    if($control_default>0) {
        $tmp_info=\App\Models\tipo_aa::find($control_default);
        if(!empty($tmp_info)) {
            if($tmp_info->nivel==1) {
...
<div class="row ">
    ...
    <div class="form-group col-sm-6 {{ $texto_resaltar_n1 }} ">
        {!! Form::label($control_control."_depto", "Actor Armado", ['class'=>$texto_resaltar_n1]) !!}
        {!! Form::select($control_control."_depto", \App\Models\tipo_aa::listar_hijos(null,$control_vacio), $tmp_id_depto,['class' => 'form-control']) !!}
    </div>
    {{-- subtipo --}}
    <div class="form-group col-sm-6 {{ $texto_resaltar_n2 }}">
        {!! Form::label($control_control, "Subtipo", ['class'=>$texto_resaltar_n2]) !!}
        {!! Form::select($control_control, \App\Models\tipo_aa::listar_hijos($tmp_id_depto,$control_vacio), $tmp_id_muni,['class' => 'form-control']) !!}
    </div>
</div>
