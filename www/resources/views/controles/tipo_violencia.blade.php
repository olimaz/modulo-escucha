...
Selector de dos niveles para Tipos de violencia
    Parametros:
...
<?php
    ...
    if($control_default>0) {
        $tmp_info=\App\Models\tipo_violencia::find($control_default);
        if(!empty($tmp_info)) {
            if($tmp_info->nivel==1) {
...
<div class="row ">
    <!-- Nivel 1 -->
    <div class="col-sm-6">
        <div class="form-group {{ $tmp_id_depto > 0 ? $texto_resaltar_n1 : "" }}">
            {!! Form::label($control_control."_depto", "Tipo", ['class'=>$texto_resaltar_n1]) !!}
            {!! Form::select($control_control."_depto", \App\Models\tipo_violencia::listar_hijos(null,$control_vacio), $tmp_id_depto,['class' => 'form-control']) !!}
        </div>
    </div>

    <!-- Nivel 2 -->
    <div class="col-sm-6">
        <div class="form-group {{ $tmp_id_muni > 0 ? $texto_resaltar_n2 : ""}}">
            {!! Form::label($control_control, "Subtipo", ['class'=>$texto_resaltar_n2]) !!}
            {!! Form::select($control_control, \App\Models\tipo_violencia::listar_hijos($tmp_id_depto,$control_vacio), $tmp_id_muni,['class' => 'form-control']) !!}
        </div>
    </div>
</div>
    ...
    @if(strlen($control_vacio)>0)
        <script>
            ...
            $("#{{ $control_control }}").depdrop({
                url: '{{ url('json/geo_todo_violencia') }}',
