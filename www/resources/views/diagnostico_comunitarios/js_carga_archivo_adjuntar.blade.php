...
@push('js')
    ...
    <script>
        ...
        function upload(img) {
            ...
            form_data.append('id_expediente',{{ $diagnosticoComunitario->id_diagnostico_comunitario }});


            $("#btn_grabar").prop("disabled",true);
            $("#btn_grabar").val(ico_cargando_boton);
            $.ajax({
                url: "{{url('/upload_adjuntar_dc')}}",
                data: form_data,
                type: 'POST',
                dataType: "json",
                contentType: false,
        ...
        function actualizar_tabla() {
            $.ajax({
                url: " {{ url("/tabla_adjuntos_dc/".$diagnosticoComunitario->id_diagnostico_comunitario) }} ",
                type: 'GET',
                success: function (data) {
