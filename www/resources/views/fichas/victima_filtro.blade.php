...
<div class="card   {{ $filtros->hay_filtro >0 ? ' collapsed-card card-success' : ' card-info ' }}">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-filter"></i> Víctimas: aplicar filtros a la información mostrada {{ $filtros->hay_filtro>0 ? ($filtros->hay_filtro==1 ? "($filtros->hay_filtro filtro aplicado)" : "($filtros->hay_filtro filtros aplicados)") : "" }}</h3>
    ...
    <div class="card-body text-sm">
        @include("fichas.partials.filtros_socio")
        <div class="row">
            <div class="col-md-6">
                @include("fichas.partials.filtros_metadatos")
            </div>
            <div class="col-md-6">
                @include("fichas.partials.filtros_buscadora")
            </div>

        </div>
        <div class="row">
            <div class="col-sm-6">
                @include("fichas.partials.filtros_participacion")
            </div>
            {{-- Datos como víctima: grupos, etc. --}}
            <div class="col-sm-6">
                @include("fichas.partials.filtros_violencia")
            </div>
        </div>
        <div class="row">
            <div class="col-sm-6">
                @include("fichas.partials.filtros_contexto")
            </div>
            {{-- Datos como víctima: grupos, etc. --}}
            <div class="col-sm-6">
                @include("fichas.partials.filtros_impactos")
            </div>
        </div>
    </div>
