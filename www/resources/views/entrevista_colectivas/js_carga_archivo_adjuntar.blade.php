...
@push('js')
    ...
    <script>
        ...
        function upload(img) {
            ...
            form_data.append('id_expediente',{{ $entrevistaColectiva->id_entrevista_colectiva }});


            $("#btn_grabar").prop("disabled",true);
            $("#btn_grabar").val(ico_cargando_boton);
            $.ajax({
                url: "{{url('/upload_adjuntar_co')}}",
                data: form_data,
                type: 'POST',
                dataType: "json",
                contentType: false,
        ...
        function actualizar_tabla() {
            $.ajax({
                url: " {{ url("/tabla_adjuntos_colectiva/".$entrevistaColectiva->id_entrevista_colectiva) }} ",
                type: 'GET',
                success: function (data) {
