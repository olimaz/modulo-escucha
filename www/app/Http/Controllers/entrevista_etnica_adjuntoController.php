...
class entrevista_etnica_adjuntoController extends Controller
{
    //
    public function quitar(Request $request)
    {
        $id=$request->id;
        $entrevista_adjunto = entrevista_etnica_adjunto::find($id);
        if (empty($entrevista_adjunto)) {
            Flash::error('Adjunto no existe');
            return redirect(action('entrevista_etnicaController@index'));
        }
        $id_entrevista = $entrevista_adjunto->id_entrevista_etnica;
        $id_adjunto    = $entrevista_adjunto->id_adjunto;

        $entrevista_adjunto->delete();

        Flash::success('Archivo adjunto eliminado.');
        traza_actividad::create(['id_objeto'=>2, 'id_accion'=>10, 'codigo'=>entrevista_etnica::find($id_entrevista)->entrevista_codigo, 'id_primaria'=>$id_adjunto,'referencia'=>"id_entrevista:$id_entrevista"]);
    ...
    public function trans($id) {
        $cual=entrevista_etnica_adjunto::find($id);
