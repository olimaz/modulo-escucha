 ...
 * @property int $tiempo_etiquetado
 * @property int $tiempo_fichas
 */
class excel_asignaciones_etiquetado extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'sim.excel_asignaciones_etiquetado';

    /**
     * @var array
     */
    protected $fillable = ['id_asignacion', 'entrevista_tipo', 'entrevista_codigo', 'entrevista_duracion_minutos', 'entrevista_macroterritorio', 'entrevista_territorio', 'entrevista_entrevistador', 'entrevista_entrevistador_grupo', 'asignacion_quien_asigna_codigo', 'asignacion_urgente', 'asignacion_responsable_codigo', 'asignacion_responsable_macroterritorio', 'asignacion_responsable_territorio', 'asignacion_responsable_grupo', 'asignacion_responsable_perfil', 'procesamiento_situacion', 'procesamiento_porque_no', 'procesamiento_terceros', 'procesamiento_observaciones', 'fecha_asignado', 'fecha_revocado', 'fecha_procesado', 'fecha_anulado', 'tiempo_entrevista', 'tiempo_transcripcion', 'tiempo_etiquetado', 'tiempo_fichas'];
    ...
    public static function generar_plana() {
        $inicio = Carbon::now();
        Log::notice("ETL de asignaciones para etiquetar: inicio del proceso");

        excel_asignaciones_etiquetado::truncate();
        ...
        Log::info("ETL de asignaciones para etiquetar: fin del proceso, $total_filas filas generadas. Tiempo: $respuesta->duracion.");
    ...
    public static function cargar_asignaciones() {
        $total_filas=0;
        $listado = etiquetar_asignacion::orderby('id_etiquetar_asignacion')->get();
        foreach($listado as $fila) {
            $entrevista = excel_asignaciones_transcripcion::traer_entrevista($fila);
            if($entrevista) {
                $excel = new excel_asignaciones_etiquetado();
                $excel->id_asignacion = $fila->id_etiquetar_asignacion;
