...
/**
 * @property int $id_diagnostico_comunitario_adjunto
 ...
 */
class diagnostico_comunitario_adjunto extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'esclarecimiento.diagnostico_comunitario_adjunto';
     ...
     * @var string
     */
    protected $primaryKey = 'id_diagnostico_comunitario_adjunto';

    /**
     * @var array
     */
    protected $fillable = ['id_diagnostico_comunitario', 'id_adjunto', 'id_usuario','id_tipo', 'created_at', 'updated_at'];
    ...
    public function getFmtTranscritoAttribute() {
        if($this->id_tipo==2) {
            ...
            elseif($this->id_transcripcion >0) {
                $trans = diagnostico_comunitario_adjunto::find($this->id_transcripcion);
                if($trans) {
                    return "Sí: $trans->url";
                }
                else {
    ...
    public function getUrlAttribute() {
        ...
        if(Storage::exists($ubica)) {
            $url = "<a target='_blank' href='".action('adjuntoController@show_dc',$adjunto->id_adjunto)."'>$adjunto->nombre</a>";
        }
        else {
            $url = "$adjunto->nombre";
        }
    ...
    public static function listado_adjuntos($id_entrevista) {
        $listado = diagnostico_comunitario_adjunto::where('id_diagnostico_comunitario',$id_entrevista)
            ->join('catalogos.criterio_fijo as cf','id_tipo','=','id_opcion')
            ->where('id_grupo',1)
            ->orderby('orden')
            ->orderby('id_diagnostico_comunitario')->get();
        $arreglo=array();
        $entrevista=diagnostico_comunitario::find($id_entrevista);
        foreach($listado as $archivo) {
            $adjunto = adjunto::find($archivo->id_adjunto);
            $item= array();
            $item['id_diagnostico_comunitario_adjunto']=$archivo->id_diagnostico_comunitario_adjunto;
            ...
            if(Storage::exists($ubica)) {
                ...
                if($visor) {
                    ...
                    $url = action('adjuntoController@show_dc',$adjunto->id_adjunto);
                }
                $item['url']="<a target='_blank' href='".$url."'>$adjunto->nombre_dc</a>";
            }
            else {
                $item['url']="$adjunto->nombre_dc";
            }
            /*
    ...
    public static function adjuntar_archivo($id_entrevista, $archivo="", $control="", $nombre_orginal="", $otr='') {
        ...
        if($entrevista) {
            ...
            $adjunto->save();
            $tmp['id_diagnostico_comunitario'] = $id_entrevista;
            $tmp['id_adjunto']=$adjunto->id_adjunto;
            $tmp['id_tipo']=$id_tipo;
            $tmp['id_usuario'] = \Auth::id() ?? 2;
            $tmp['created_at']= Carbon::now();
            $adjunto_rel = diagnostico_comunitario_adjunto::create($tmp);
            ...
            if(strlen($otr)>0) {
                ...
                $adjunto->save();
                $tmp['id_adjunto']=$adjunto->id_adjunto;
                $adjunto_rel2 = diagnostico_comunitario_adjunto::create($tmp);
                //Registrar traza
    ...
    public function procesar_etiquetas() {
        ...
        $adjunto=$this->rel_id_adjunto;
        if($adjunto) {
            $entrevista = $this->rel_id_diagnostico_comunitario;
            $etiquetas = $adjunto->procesar_json_etiquetado();
            if($etiquetas->exito) {
                etiqueta_entrevista::where('id_subserie',config('expedientes.dc'))->where('id_entrevista',$this->id_diagnostico_comunitario)->delete();
                ...
                foreach($etiquetas->a_marcas as $etiqueta => $a_marca) {
                    ...
                    $nuevo['id_entrevista']=$this->id_diagnostico_comunitario;
                    $nuevo['id_subserie']=config('expedientes.dc');
