...
/**
 * @property int $id_entrevista_profundidad_adjunto
 ...
 */
class entrevista_profundidad_adjunto extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'esclarecimiento.entrevista_profundidad_adjunto';
     ...
     * @var string
     */
    protected $primaryKey = 'id_entrevista_profundidad_adjunto';

    /**
     * @var array
     */
    protected $fillable = ['id_entrevista_profundidad', 'id_adjunto','id_tipo', 'id_usuario', 'created_at', 'updated_at'];
    ...
    public function getFmtTranscritoAttribute() {
        if($this->id_tipo==2) {
            ...
            elseif($this->id_transcripcion >0) {
                $trans = entrevista_profundidad_adjunto::find($this->id_transcripcion);
                if($trans) {
                    return "Sí: $trans->url";
                }
                else {
    ...
    public function getUrlAttribute() {
        ...
        if(Storage::exists($ubica)) {
            $url = "<a target='_blank' href='".action('adjuntoController@show_pr',$adjunto->id_adjunto)."'>$adjunto->nombre</a>";
        }
        else {
            $url = "$adjunto->nombre";
        }
    ...
    public static function listado_adjuntos($id_entrevista) {
        $listado = entrevista_profundidad_adjunto::where('id_entrevista_profundidad',$id_entrevista)
            ->join('catalogos.criterio_fijo as cf','id_tipo','=','id_opcion')
            ->where('id_grupo',1)
            ->orderby('orden')
            ->orderby('id_entrevista_profundidad_adjunto')->get();
        $arreglo=array();
        $entrevista = entrevista_profundidad::find($id_entrevista);
        foreach($listado as $archivo) {
            $adjunto = adjunto::find($archivo->id_adjunto);
            $item= array();
            $item['id_entrevista_profundidad_adjunto']=$archivo->id_entrevista_profundidad_adjunto;
            ...
            if(Storage::exists($ubica)) {
                ...
                if($visor) {
                    ...
                    $url =action('adjuntoController@show_pr',$adjunto->id_adjunto);
                }
                $item['url']="<a target='_blank' href='".$url."'>$adjunto->nombre_pr</a>";
            }
            else {
                $item['url']="$adjunto->nombre_pr";
            }
            /*
    ...
    public static function adjuntar_archivo($id_entrevista, $archivo="", $control="", $nombre_orginal="", $otr='') {
        ...
        if($entrevista) {
            ...
            $adjunto->save();
            $tmp['id_entrevista_profundidad'] = $id_entrevista;
            $tmp['id_adjunto']=$adjunto->id_adjunto;
            $tmp['id_tipo']=$id_tipo;
            $tmp['created_at']= Carbon::now();
            $adjunto_rel = entrevista_profundidad_adjunto::create($tmp);
            ...
            if(strlen($otr)>0) {
                ...
                $adjunto->save();
                $tmp['id_adjunto']=$adjunto->id_adjunto;
                $adjunto_rel2 = entrevista_profundidad_adjunto::create($tmp);
                //Registrar traza
    ...
    public static function calificar($entrevista, $id_tipo) {
        ...
        elseif($entrevista->id_tipo==5) { //Compareciente
            $res->id_calificacion=3;
    ...
    /*
     * Transcripcion por GOOGLE
     */


    public function transcribir() {
        $adjunto =$this->rel_id_adjunto;
        $entrevista = $this->rel_id_entrevista_profundidad;
        ...
        $cola->id_subserie = config('expedientes.pr');
        $cola->id_primaria = $this->id_entrevista_profundidad_adjunto;
    ...
    public function transcribir_revisar() {
        ...
        $control = control_transcripcion::where('id_subserie',$id_subserie)
            ->where('id_primaria',$this->id_entrevista_profundidad_adjunto)
        ...
        if($control->id_estado==2) { //En cola: revisar
            ...
            if(!$estado->transcrito) {  //No transcrito
                ...
                $nuevo = entrevista_profundidad_adjunto::firstOrcreate($adjuntado);
                $this->id_transcripcion=$nuevo->id_entrevista_profundidad_adjunto;
                $this->save();
                //Actualizar el control
                $control->id_estado=3;
                $control->fh_fin=Carbon::now();
                $control->id_adjunto_nuevo = $nuevo->id_entrevista_profundidad_adjunto;
    ...
    public static function quitar_trans($id) {
        ...
        $control = control_transcripcion::where('id_subserie',$id_subserie)
            ...
            ->orderby('id_control_transcripcion','desc')
            ->first();
        $adjuntado = entrevista_profundidad_adjunto::find($id);
        $transcripcion = entrevista_profundidad_adjunto::find($adjuntado->id_transcripcion);
        if($transcripcion) {
    ...
    public static function revisar_bloque_google($bloque=0) {
        ...
        $respuesta=new \stdClass();
        $respuesta->titulo="Transcripciones PR";
        ...
        $sql_sin_transcripcion="select distinct a.id_entrevista_profundidad_adjunto
                            from esclarecimiento.entrevista_profundidad e join esclarecimiento.entrevista_profundidad_adjunto a on (e.id_entrevista_profundidad=a.id_entrevista_profundidad)
                            where a.id_tipo=2 and a.id_transcripcion is null
                                -- Transcritos a mano
                              and a.id_entrevista_profundidad not in (
                                  select e.id_entrevista_profundidad
                                        from esclarecimiento.entrevista_profundidad e join public.transcribir_asignacion a on (e.id_entrevista_profundidad=a.id_entrevista_profundidad)
                                        where a.id_situacion=2                    
                                )
                            order by 1";
        $sql_revisar =  "select distinct a.id_entrevista_profundidad_adjunto
                            from esclarecimiento.entrevista_profundidad e join esclarecimiento.entrevista_profundidad_adjunto a on (e.id_entrevista_profundidad=a.id_entrevista_profundidad)
        ...
        foreach($listado_revisar as $fila) {
            $pendientes_revisar[]=$fila->id_entrevista_profundidad_adjunto;
        }
        ...
        foreach($pendientes_revisar as $cual) {
            ...
            $adjuntado = entrevista_profundidad_adjunto::find($cual);
        ...
        if(count($respuesta->cola_revisar)==0) {
            ...
            foreach($listado_transcribir as $fila) {
                $respuesta->log[]="Para la cola: ".$fila->id_entrevista_profundidad_adjunto;
                $listado_enviar[]=$fila->id_entrevista_profundidad_adjunto;
                $maximo++;
                if($maximo > $bloque) {
                    break;
                }
    ...
    public function procesar_etiquetas() {
        ...
        if($adjunto) {
            ...
            if($etiquetas->exito) {
                etiqueta_entrevista::where('id_subserie',config('expedientes.pr'))->where('id_entrevista',$this->id_entrevista_profundidad)->delete();
                ...
                foreach($etiquetas->a_marcas as $etiqueta => $a_marca) {
                    ...
                    $nuevo['id_entrevista']=$this->id_entrevista_profundidad;
                    $nuevo['id_subserie']=config('expedientes.pr');
