...
/**
 * @property int $id_entrevista_colectiva_adjunto
 ...
 * @property entrevistaColectiva entrevistaColectiva
 * @property adjunto adjunto
 */
class entrevista_colectiva_adjunto extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'esclarecimiento.entrevista_colectiva_adjunto';
     ...
     * @var string
     */
    protected $primaryKey = 'id_entrevista_colectiva_adjunto';

    /**
     * @var array
     */
    protected $fillable = ['id_entrevista_colectiva', 'id_adjunto', 'id_usuario','id_tipo','id_transcripcion' ,'created_at', 'updated_at'];
    ...
    public function getFmtTranscritoAttribute() {
        if($this->id_tipo==2) {
            ...
            elseif($this->id_transcripcion >0) {
                $trans = entrevista_colectiva_adjunto::find($this->id_transcripcion);
                if($trans) {
                    return "Sí: $trans->url";
                }
                else {
    ...
    public function getUrlAttribute() {
        ...
        if(Storage::exists($ubica)) {
            $url = "<a target='_blank' href='".action('adjuntoController@show_co',$adjunto->id_adjunto)."'>$adjunto->nombre</a>";
        }
        else {
            $url = "$adjunto->nombre";
        }
    ...
    public static function listado_adjuntos($id_entrevista) {
        $listado = entrevista_colectiva_adjunto::where('id_entrevista_colectiva',$id_entrevista)
            ->join('catalogos.criterio_fijo as cf','id_tipo','=','id_opcion')
            ->where('id_grupo',1)
            ->orderby('orden')
            ->orderby('id_entrevista_colectiva_adjunto')->get();
        $arreglo=array();
        $entrevista = entrevista_colectiva::find($id_entrevista);
        foreach($listado as $archivo) {
            $adjunto = adjunto::find($archivo->id_adjunto);
            $item= array();
            $item['id_entrevista_colectiva_adjunto']=$archivo->id_entrevista_colectiva_adjunto;
            $item['id_adjunto']=$adjunto->id_adjunto;
            $item['adjunto']=$adjunto;
            //$item['nombre']=$adjunto->nombre_co;
            $item['nombre']=$adjunto->nombre_co;
            ...
            if(Storage::exists($ubica)) {
                ...
                if($visor) {
                    ...
                    $url = action('adjuntoController@show_co',$adjunto->id_adjunto);
                }
                $item['url']="<a target='_blank' href='".$url."'>$adjunto->nombre_co</a>";
    ...
    public static function adjuntar_archivo($id_entrevista, $archivo="", $control="", $nombre_orginal="", $otr='') {
        ...
        if($entrevista) {
            ...
            $adjunto->save();
            $tmp['id_entrevista_colectiva'] = $id_entrevista;
            $tmp['id_adjunto']=$adjunto->id_adjunto;
            $tmp['id_tipo']=$id_tipo;

            $tmp['created_at']= Carbon::now();
            $tmp['id_usuario'] = \Auth::id() ?? 2;
            $adjunto_rel = entrevista_colectiva_adjunto::create($tmp);
            ...
            if(strlen($otr)>0) {
                ...
                $adjunto->save();
                $tmp['id_adjunto']=$adjunto->id_adjunto;
                $adjunto_rel2 = entrevista_colectiva_adjunto::create($tmp);
                //Justificar la calificacion
                foreach($calificacion->justificacion as $id_justificacion) {
    ...
    public function procesar_etiquetas() {
        ...
        $adjunto=$this->rel_id_adjunto;
        if($adjunto) {
            $entrevista = $this->rel_id_entrevista_colectiva;
            $etiquetas = $adjunto->procesar_json_etiquetado();
            if($etiquetas->exito) {
                etiqueta_entrevista::where('id_subserie',config('expedientes.co'))->where('id_entrevista',$this->id_entrevista_colectiva)->delete();
                ...
                foreach($etiquetas->a_marcas as $etiqueta => $a_marca) {
                    ...
                    $nuevo['id_entrevista']=$this->id_entrevista_colectiva;
                    $nuevo['id_subserie']=config('expedientes.co');
