 ...
 * @property string $tema_fecha_del
 * @property string $tema_fecha_al
...
class excel_co extends Model
{
    ...
    public static function generar_plana() {
        $inicio = Carbon::now();
        Log::notice("ETL de entrevistas colectivas: inicio del proceso");
        ...
        //Registro
        Log::info("ETL de entrevistas colectivas: fin del proceso, $total_filas filas generadas, $total_errores filas con problemas. Tiempo: $respuesta->duracion.");
        if ($total_errores > 0) {
            Log::error("Problemas en el ETL de entrevistas colectivas");
        }
        //
        return $respuesta;
    }
    ...
    //Entrevistas colectivos
    public static function cargar_entrevistas() {
        $total_filas=0;
        $total_errores=0;
        $listado = entrevista_colectiva::where('id_activo',1)->orderby('entrevista_correlativo')->get();

        foreach($listado as $fila) {
            $excel = new excel_co();
            $excel->id_entrevista_colectiva = $fila->id_entrevista_colectiva;
            ...
            $excel->eventos = $fila->eventos_descripcion;
