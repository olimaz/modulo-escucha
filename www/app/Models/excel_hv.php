 ...
 * @property string $entrevistado_sexo
 * @property string $entrevistado_orientacion_sexual
 * @property string $entrevistado_identidad_genero
 * @property string $entrevistado_pertenencia_etnico_racial
...
class excel_hv extends Model
{
    ...
    public static function generar_plana() {
        $inicio = Carbon::now();
        Log::notice("ETL de historias de vida: inicio del proceso");
        ...
        //Registro
        Log::info("ETL de historias de vida: fin del proceso, $total_filas filas generadas, $total_errores filas con problemas. Tiempo: $respuesta->duracion.");
        if ($total_errores > 0) {
            Log::error("Problemas en el ETL de historias de vida");
        }
        //
        return $respuesta;
    }
    ...
    public static function cargar_entrevistas() {
        $total_filas=0;
        $total_errores=0;
        $listado = historia_vida::where('id_activo',1)->orderby('entrevista_correlativo')->get();

        foreach($listado as $fila) {
            $excel = new excel_hv();
            $excel->id_historia_vida = $fila->id_historia_vida;
            ...
            $excel->entrevistado_otros_nombres = $fila->entrevistado_otros_nombres;
            $excel->entrevistado_sexo = $fila->fmt_id_sexo;
            $excel->entrevistado_orientacion_sexual = $fila->fmt_id_orientacion_sexual;
            $excel->entrevistado_identidad_genero = $fila->fmt_id_identidad_genero;
            $excel->entrevistado_pertenencia_etnico_racial = $fila->fmt_id_pertenencia_etnico_racial;
            ...
            try {
                ...
                $total_errores++;
                Log::debug("Error en generar registro de excel_hv. ". PHP_EOL .$e->getMessage());
            }
        }
        $res['si']=$total_filas;
