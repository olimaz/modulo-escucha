 ...
 * @property string $lugar_nac_n2_txt
 ...
 */
class analitica_victima extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'analitica.victima';
     ...
     * @var array
     */
    protected $fillable = ['id_entrevista', 'id_persona', 'codigo_entrevista', 'nombre', 'apellido', 'otros_nombres', 'fec_nac_anio', 'fec_nac_mes', 'fec_nac_dia', 'lugar_nac_codigo', 'lugar_nac_n1_codigo', 'lugar_nac_n1_txt', 'lugar_nac_n2_codigo', 'lugar_nac_n2_txt', 'lugar_nac_n3_codigo', 'lugar_nac_n3_txt', 'lugar_nac_n3_lat', 'lugar_nac_n3_lon', 'sexo_id', 'sexo_txt', 'orientacion_sexual_id', 'orientacion_sexual_txt', 'identidad_genero_id', 'identidad_genero_txt', 'pertenencia_etnica_id', 'pertenencia_etnica_txt', 'pertenencia_indigena_id', 'pertenencia_indigena_txt', 'documento_identidad_tipo_id', 'documento_identidad_tipo_txt', 'documento_identidad_numero', 'nacionalidad_id', 'nacionalidad_txt', 'estado_civil_id', 'estado_civil_txt', 'lugar_residencia_codigo', 'lugar_residencia_n1_codigo', 'lugar_residencia_n1_txt', 'lugar_residencia_n2_codigo', 'lugar_residencia_n3_codigo', 'lugar_residencia_n3_txt', 'lugar_residencia_n3_lat', 'lugar_residencia_n3_lon', 'lugar_residencia_zona_id', 'lugar_residencia_zona_txt', 'lugar_residencia_descripcion', 'educacion_formal_id', 'educacion_formal_txt', 'profesion', 'ocupacion_actual', 'discapacidad_id', 'discapacidad_txt', 'cargo_publico', 'cargo_publico_cual', 'fuerza_publica_miembro', 'fuerza_publica_estado', 'fuerza_publica_especificar', 'actor_armado_ilegal', 'actor_armado_ilegal_especificar', 'organizacion_colectivo_participa', 'insert_id_entrevistador', 'insert_fecha_hora', 'insert_fecha', 'insert_fecha_mes', 'update_id_entrevistador', 'update_fecha_hora'];
    ...
    */
    public static function generar_plana() {
        $inicio = Carbon::now();
        //Registrar el evento
        Log::notice("ETL de analitica-victimas: inicio del proceso");
        //Inicializar la tabla
        analitica_victima::truncate();
        ...
        foreach($listado as $fila) {
            ...
            //Crear registro
            $excel = new analitica_victima();
            $excel->id_victima = $fila->id_victima;
            $excel->id_duplicado = $fila->id_duplicado;
            ...
            //Relacion
            $persona_entrevistada = $entrevista->rel_ficha_persona_entrevistada;
            if($persona_entrevistada) {
                $parentezco = \DB::table('fichas.per_ent_rel_victima')->where('id_victima',$fila->id_victima)->where('id_persona_entrevistada',$persona_entrevistada->id_persona_entrevistada)->first();
                if($parentezco) {
                    $excel->relacion_persona_entrevistada = cat_item::describir($parentezco->id_rel_victima);
                    $excel->id_persona_entrevistada = $persona_entrevistada->id_persona_entrevistada;
                }
            }
            ...
            */
            ...
            if(!is_null($fila->updated_at)) {
                $fecha = Carbon::createFromFormat('Y-m-d H:i:s',$fila->updated_at);
            ...
            try {
                ...
                $total_errores++;
                Log::debug("Error en generar registro de analitica-victimas: ".$e->getMessage());
            }
        }
        ...
        Log::info("ETL de analitica-victimas: fin del proceso, $total_filas filas generadas. Tiempo: $respuesta->duracion.");
        if($total_errores>0) {
            Log::error("ETL de analitica-victimas finalizada con  $total_errores errores");
        }
